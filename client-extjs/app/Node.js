Ext.define('FileBot.Node', {
    ...
    init_generic: function() {
        ...
        FileBot.getApplication().on('schedule', function(request) {
            ...
            Ext.MessageBox.show({
                title: 'Prepared Task',
                msg: '<span class="crontab">Prepared Task ' + id + ' can be called locally via <nobr><code>' + command + '</code></nobr><br/> or remotely via <a href="data:text/plain,' + encodeURIComponent(curl) + '" download="curl.sh">' + url + '</a>.</span>',
                buttons: Ext.MessageBox.OK,
